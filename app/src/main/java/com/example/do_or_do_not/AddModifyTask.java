...
public class AddModifyTask extends AppCompatActivity {
    ...
    public void init_modify() {
        ...
        Cursor task = db.getSingleTask(task_id);
        if (task != null) {
            task.moveToFirst();
    ...
    public void saveTask(View v) {
        ...
        if (edit_text.getText().toString().trim().length() > 0) {
            ...
            if (isModify) {
                db.updateTask(task_id, edit_text.getText().toString(), new SimpleDateFormat("yyyy-MM-dd").format(calendar.getTime()));
                Toast.makeText(getApplicationContext(), "Task Updated.", Toast.LENGTH_SHORT).show();
            } else {
                db.insertTask(edit_text.getText().toString(), new SimpleDateFormat("yyyy-MM-dd").format(calendar.getTime()));
